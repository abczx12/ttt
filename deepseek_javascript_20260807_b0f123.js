...
// UI TO HƠN + SAO BĂNG RƠI + BÓP MÉO TIẾNG
...
(function () {
    ...
    function createShootingStars() {
        ...
        class ShootingStar {
            ...
            reset() {
                ...
                this.tailLength = 15 + Math.random() * 20;
            }
            ...
            update() {
                if (!this.active) {
                    if (Math.random() < 0.003) this.start();
                    return;
                }
            ...
            draw(ctx) {
                ...
                for (let i = 1; i < this.trail.length; i++) {
                    ...
                    ctx.shadowColor = `rgba(100, 200, 255, ${alpha * 0.3})`;
                    ctx.shadowBlur = 10 * (i / this.trail.length);
                    ctx.fill();
                    ctx.shadowBlur = 0;
                }
                ...
                gradient.addColorStop(0.5, `rgba(200, 230, 255, ${this.opacity * 0.5})`);
                gradient.addColorStop(1, `rgba(100, 200, 255, 0)`);
                
                ctx.beginPath();
                ctx.arc(last.x, last.y, 8, 0, Math.PI * 2);
                ctx.fillStyle = gradient;
                ctx.shadowColor = `rgba(100, 200, 255, ${this.opacity * 0.3})`;
                ctx.shadowBlur = 30;
                ctx.fill();
                ctx.shadowBlur = 0;
            }
    ...
    const Core = {
        ...
        push() {
            ...
            mp.get('distortion').setTargetAtTime(P.distortion, t, 0.015);
            mp.get('fuzz').setTargetAtTime(P.fuzz, t, 0.015);
            mp.get('octave').setTargetAtTime(P.octave, t, 0.015);
            mp.get('bitcrush').setTargetAtTime(P.bitcrush, t, 0.015);
        }
    };
        ...
        init() {
            ...
            <div id="kh-head" style="display:flex;justify-content:space-between;align-items:center;padding:16px 24px;background:rgba(0,0,0,0.3);border-bottom:1px solid rgba(100,200,255,0.05);border-radius:20px 20px 0 0;">
                <div id="kh-title" style="display:flex;align-items:center;gap:14px;">
                    ...
                    <span id="kh-name" style="font-size:20px;font-weight:900;letter-spacing:1px;text-shadow:0 0 30px rgba(0,150,255,0.3);">COSMIC</span>
                </div>
    ...
    console.log('🌌 UI to hơn + Sao băng rơi');
