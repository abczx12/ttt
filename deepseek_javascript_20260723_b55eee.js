// hellfire_amduong.js - Bố Duy Khánh ÂM DƯƠNG EDITION
// PHÁT MP3 TO THEO CONFIG + SÓNG NHẢY + UI ÂM DƯƠNG

(function () {
    'use strict';
    if (window.__BDK_AMDUONG__) return;
    window.__BDK_AMDUONG__ = true;

    console.log('%c☯️ HELLFIRE ÂM DƯƠNG - Bố Duy Khánh', 'font-size:24px;font-weight:bold;color:#ffffff;text-shadow:0 0 40px #888;');
    ...
    const P = {
        ...
        gateThreshold: 0.008,
        deEsser: 0.3,
        musicVolume: 0.5, // Volume nhạc riêng (0-1)
        musicSync: true,  // Tự động đồng bộ với config?
    };
    ...
    function showToast(message, type = 'info', duration = 2500) {
        ...
        toast.style.cssText = `
            ...
            box-shadow:0 0 40px ${colors[type] || '#888'}20, inset 0 0 40px ${colors[type] || '#888'}10;
            z-index:9999999;
            font-size:14px;
            font-weight:bold;
    ...
    // ============================================================
    //  HOOK HỆ THỐNG - GIỮ NGUYÊN HELLFIRE
    // ============================================================
    ...
    // ============================================================
    //  APPLY CONFIG + UPDATE MUSIC VOLUME
    // ============================================================
    function applyConfig(config) {
        Object.assign(P, config);
        
        // Nếu bật sync, tự động cập nhật musicVolume theo postGain
        if (P.musicSync !== false) {
            // postGain thường từ 1-20, quy về 0-1
            const maxPostGain = 20;
            const normalizedVolume = Math.min(1, P.postGain / maxPostGain);
            P.musicVolume = Math.max(0.05, normalizedVolume);
            
            // Cập nhật slider volume nếu có
            const mvSlider = document.getElementById('sl-mv');
            const mvLabel = document.getElementById('lb-mv');
            if (mvSlider) {
                mvSlider.value = P.musicVolume * 100;
                mvSlider.style.setProperty('--v', (P.musicVolume * 100) + '%');
            }
            if (mvLabel) {
                mvLabel.innerText = (P.musicVolume * 100).toFixed(0) + '%';
            }
            
            // Cập nhật music player
            updateMusicVolume(P.musicVolume);
            showToast(`🔊 Nhạc đã đồng bộ với config (${(P.musicVolume * 100).toFixed(0)}%)`, 'success');
        }
    ...
    // ============================================================
    //  AUDIO VISUALIZER - SÓNG NHẢY TRONG GUI
    // ============================================================
    ...
    // ============================================================
    //  MUSIC PLAYER - PHÁT MP3 + ĐỒNG BỘ VOLUME VỚI CONFIG
    // ============================================================
    ...
    function toggleMusic() {
        ...
        try {
            ...
            // Lấy volume từ P (đã được đồng bộ với config)
    ...
    // ============================================================
    //  UI - ÂM DƯƠNG (TRẮNG ĐEN) + GLOW + VIỀN ĐUỔI MÀU
    // ============================================================
    const UI = {
        badge(t, c) {
        ...
        init() {
            // Nút toggle ☯️ với hiệu ứng xoay
            ...
            <div id="kh-head">
                <div id="kh-title">
                    ...
                    <span id="kh-name" class="kh-glow-text">ÂM DƯƠNG</span>
                </div>
                <div id="kh-right">
                    <div id="kh-badge">
                        <span id="kh-dot"></span>
            ...
            <div id="kh-body">
                ...
                <select id="config-select" style="width:100%;padding:10px 12px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.1);color:#fff;border-radius:8px;font-size:14px;cursor:pointer;margin-bottom:10px;">
                ...
                <div style="display:flex;gap:6px;margin-bottom:12px;align-items:center;flex-wrap:wrap;">
                    <input type="file" id="music-file" accept="audio/*" style="flex:1;min-width:120px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.1);color:#fff;border-radius:4px;padding:4px 8px;font-size:11px;">
                    <button id="music-play" style="padding:4px 12px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#fff;border-radius:4px;cursor:pointer;font-size:11px;white-space:nowrap;">▶ PLAY</button>
                    <span id="music-status" style="font-size:10px;color:#666;min-width:80px;">Chọn file</span>
                </div>
                
                <!-- VOLUME SLIDER CHO NHẠC -->
                <div style="display:flex;gap:8px;align-items:center;margin-bottom:12px;padding:8px 12px;background:rgba(255,255,255,0.02);border-radius:6px;border:1px solid rgba(255,255,255,0.03);">
                    ...
                    <input type="range" id="sl-mv" min="0" max="100" step="1" value="${(P.musicVolume || 0.5) * 100}" style="flex:1;--v:${(P.musicVolume || 0.5) * 100}%;">
                    <span id="lb-mv" style="font-size:11px;color:#fff;min-width:40px;text-align:right;">${((P.musicVolume || 0.5) * 100).toFixed(0)}%</span>
                    <button id="btn-sync-music" style="padding:2px 8px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#888;border-radius:4px;cursor:pointer;font-size:10px;white-space:nowrap;">🔄 SYNC</button>
                </div>
                    ...
                    <div class="kh-row">
                        <div class="kh-rowlabel">
                            ...
                            <div style="display:flex;gap:8px;align-items:center;">
                                ...
                                <input type="number" id="${numId}" value="${defaultVal}" style="width:80px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.1);color:#fff;border-radius:6px;padding:4px 8px;font-size:12px;text-align:center;">
                            </div>
                        </div>
            ...
            document.getElementById('kh-rst').onclick = () => {
                applyConfig({ preGain: 1, drive: 0, crush: 0, width: 0, postGain: 1, bass: 0, treble: 0, echo: 0, gateThreshold: 0.005, deEsser: 0, musicVolume: 0.5 });
            ...
            // Thông báo chào mừng
            setTimeout(() => {
                showToast('☯️ HELLFIRE ÂM DƯƠNG - Sẵn sàng!', 'info', 3000);
            }, 1000);
        },
        css() {
            ...
            @keyframes glow { 0%,100%{text-shadow:0 0 20px #888;} 50%{text-shadow:0 0 40px #fff,0 0 80px rgba(255,255,255,0.1);} }
            @keyframes yinyangPulse { 0%,100%{text-shadow:0 0 20px rgba(255,255,255,0.1);} 50%{text-shadow:0 0 60px rgba(255,255,255,0.3);} }
            @keyframes borderChase { 0% { border-color: #fff; } 25% { border-color: #888; } 50% { border-color: #fff; } 75% { border-color: #444; } 100% { border-color: #fff; } }
            ...
            #kh-root {
                ...
                border-radius:20px;
                box-shadow:0 0 60px rgba(0,0,0,0.8), inset 0 0 60px rgba(255,255,255,0.02);
            ...
            #kh-name{font-size:20px;font-weight:900;letter-spacing:1px;animation:glow 3s infinite;text-shadow:0 0 20px rgba(255,255,255,0.2);}
            ...
            .kh-rowlabel span:last-child{color:#fff;text-shadow:0 0 20px rgba(255,255,255,0.1);}
            ...
            .kh-rowlabel input[type=number]:focus{outline:none;border-color:rgba(255,255,255,0.2);box-shadow:0 0 20px rgba(255,255,255,0.02);}
    ...
    console.log('✅ HELLFIRE ÂM DƯƠNG - Bố Duy Khánh đã sẵn sàng!');
