// hellfire_amduong_plus.js - Bố Duy Khánh ÂM DƯƠNG PLUS
// HIỆU ỨNG HẠT TỤ KHÍ + ĐỔI MÀU + CHẤN ĐỘNG + BÁT QUÁI XOAY

(function () {
    'use strict';
    if (window.__BDK_AMDUONG_PLUS__) return;
    window.__BDK_AMDUONG_PLUS__ = true;

    console.log('%c☯️ HELLFIRE ÂM DƯƠNG PLUS - Bố Duy Khánh', 'font-size:24px;font-weight:bold;color:#ffffff;text-shadow:0 0 40px #888;');
    ...
    // ============================================================
    //  COLOR PALETTES (Nguyên tố / Pháp khí)
    // ============================================================
    const PALETTES = {
    ...
    const CONFIG_LIST = [
        ...
        { name: '💀 Lấn Át Cực Đỉnh', desc: 'Bão hòa max, át hết', preGain: 500, drive: 1.0, crush: 0.6, width: 0.9, postGain: 8, bass: 1.0, treble: 0.7, echo: 0.35, gateThreshold: 0.001, deEsser: 0.02 },
        { name: '⚡ Lấn Át Tối Thượng', desc: 'Át hoàn toàn, ko thở', preGain: 700, drive: 1.0, crush: 0.8, width: 1.0, postGain: 10, bass: 1.0, treble: 0.8, echo: 0.5, gateThreshold: 0.001, deEsser: 0.01 },
        ...
        { name: '💀 Thần Chết', desc: 'Giọng của thần chết, lạnh lùng', preGain: 450, drive: 0.8, crush: 0.9, width: 0.4, postGain: 8, bass: 0.8, treble: 0.2, echo: 0.3, gateThreshold: 0.002, deEsser: 0.05 },
    ];
    ...
    function getPaletteForConfig(configName) {
        if (configName.includes('Lấn Át')) return PALETTES['Hỏa Diệm'];
        if (configName.includes('Loa Phường') || configName.includes('Bom Nổ')) return PALETTES['Hoàng Kim'];
        if (configName.includes('Ác Quỷ')) return PALETTES['Âm Ty'];
        if (configName.includes('Echo')) return PALETTES['Băng Giá'];
        if (configName.includes('Thần Chết')) return PALETTES['Âm Ty'];
    ...
    // ============================================================
    //  EFFECT: PARTICLE CONVERGENCE (Tụ khí hạt)
    // ============================================================
    function runParticleConvergence(targetX, targetY, onComplete) {
        const overlay = document.createElement('canvas');
        overlay.id = 'kh-particle-overlay';
        ...
        function resize() {
            width = overlay.width = window.innerWidth;
            height = overlay.height = window.innerHeight;
        }
        resize();
        ...
        for (let i = 0; i < particleCount; i++) {
            particles.push({
                ...
                targetX: targetX + (Math.random() - 0.5) * 200,
        ...
        function draw() {
            ...
            for (const p of particles) {
                ...
                // Vẽ hạt
                ctx.beginPath();
                ctx.arc(p.x, p.y, p.size * (0.5 + 0.5 * ease), 0, Math.PI * 2);
                const alpha = p.alpha * (0.3 + 0.7 * ease);
                const brightness = Math.round(200 + 55 * ease);
                ctx.fillStyle = `rgba(${brightness}, ${brightness}, ${brightness}, ${alpha})`;
                ctx.shadowColor = `rgba(255,255,255,${0.2 * ease})`;
                ctx.shadowBlur = 20 * ease;
                ctx.fill();
                ctx.shadowBlur = 0;
            }

            if (!converged || progress < 1) {
                requestAnimationFrame(draw);
            } else {
                allConverged = true;
                // Xóa overlay sau 200ms
                setTimeout(() => {
                    overlay.remove();
    ...
    // ============================================================
    //  EFFECT: SCREEN SHAKE (Chấn động màn hình)
    // ============================================================
    ...
    // ============================================================
    //  MUSIC PLAYER - PHÁT MP3 + ĐỒNG BỘ VOLUME
    // ============================================================
    ...
    // ============================================================
    //  UI - ÂM DƯƠNG PLUS (Có Particle + Shake + Bagua)
    // ============================================================
    const UI = {
        badge(t, c) {
        ...
        init() {
            // Tạo style cho Bát Quái xoay + shake
            ...
            style.textContent = `
                ...
                #kh-toggle.active .bagua-ring {
                    animation: fastRotate 1.2s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
                }
                :root {
                    --kh-primary: #ffffff;
                    --kh-glow: rgba(255,255,255,0.3);
                    --kh-bg: #0a0a0a;
                }
                #kh-root {
                    border-color: var(--kh-primary) !important;
                    box-shadow: 0 0 60px var(--kh-glow), inset 0 0 60px var(--kh-glow) !important;
                }
                #kh-dot {
                    background: var(--kh-primary) !important;
                    box-shadow: 0 0 20px var(--kh-glow) !important;
                }
                #kh-name {
                    text-shadow: 0 0 40px var(--kh-glow) !important;
                }
                .kh-glow-text {
                    text-shadow: 0 0 20px var(--kh-glow) !important;
                }
            `;
            document.head.appendChild(style);

            // Nút toggle ☯️ với Bát Quái
            const toggleBtn = document.createElement('button');
            toggleBtn.id = 'kh-toggle';
            toggleBtn.innerHTML = `<span class="bagua-ring">☯</span>`;
            toggleBtn.style.cssText = `
                position:fixed;bottom:24px;left:24px;z-index:999999;
                width:60px;height:60px;border-radius:50%;
                background:linear-gradient(135deg,#1a1a1a,#2a2a2a);
                border:2px solid var(--kh-primary, #888);
            ...
            <div id="kh-head">
                <div id="kh-title">
                    <span class="bagua-ring" style="font-size:20px;display:inline-block;animation:slowRotate 12s linear infinite;">☯</span>
                    <span id="kh-name" class="kh-glow-text">ÂM DƯƠNG PLUS</span>
                </div>
                <div id="kh-right">
                    <div id="kh-badge">
                        <span id="kh-dot"></span>
            ...
            toggleBtn.onclick = () => {
                if (!uiVisible && firstOpen) {
                    // Lần đầu mở → chạy hiệu ứng tụ khí hạt
                    const rect = el.getBoundingClientRect();
                    const cx = rect.left + rect.width / 2;
                    const cy = rect.top + rect.height / 2;
                    runParticleConvergence(cx, cy, () => {
                        el.style.display = 'block';
                        el.style.opacity = '1';
                        uiVisible = true;
                        toggleBtn.classList.add('active');
                        toggleBtn.style.borderColor = 'var(--kh-primary, #888)';
                        setTimeout(() => toggleBtn.classList.remove('active'), 1200);
                        firstOpen = false;
                    });
                    return;
                }
                ...
                toggleBtn.style.borderColor = uiVisible ? 'var(--kh-primary, #888)' : '#333';
            };
            ...
            document.getElementById('config-select').onchange = function() {
                ...
                if (config) {
                    applyConfig(config);
                    document.getElementById('config-info').innerHTML = `✅ Đã áp dụng: <b style="color:var(--kh-primary, #fff);">${config.name}</b> - ${config.desc}`;
                    document.getElementById('config-info').style.borderColor = 'var(--kh-glow, rgba(255,255,255,0.2))';
                }
            };
            ...
            // Âm dương xoay liên tục trên toggle
            let angle = 0;
            setInterval(() => {
                angle += 0.02;
                if (toggleBtn) {
                    toggleBtn.style.transform = `rotate(${angle}deg)`;
                }
            }, 50);
            ...
            setTimeout(() => {
                showToast('☯️ HELLFIRE ÂM DƯƠNG PLUS - Sẵn sàng!', 'info', 3000);
            }, 1000);
        },
        css() {
    ...
    console.log('✅ HELLFIRE ÂM DƯƠNG PLUS - Bố Duy Khánh đã sẵn sàng!');
    console.log('☯️ Hiệu ứng tụ khí hạt + Đổi màu nguyên tố + Chấn động màn hình');
    console.log('🔄 Bát Quái xoay liên tục + Xoay gia tốc khi tương tác');
