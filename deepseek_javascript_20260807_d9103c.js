// hellfire_cosmic_STABLE.js - Bố Duy Khánh COSMIC STABLE
// FIX: Ngắt âm thanh sau 1-2 giây - GIỮ MIC LUÔN SỐNG

(function () {
    'use strict';
    if (window.__BDK_COSMIC_STABLE__) return;
    window.__BDK_COSMIC_STABLE__ = true;

    console.log('%c🌌 HELLFIRE COSMIC STABLE - Bố Duy Khánh', 'font-size:28px;font-weight:bold;color:#ff6b6b;text-shadow:0 0 60px #ff6b6b, 0 0 120px #ff6b6b;');

    // ============================================================
    //  1. HIỆU ỨNG SAO BĂNG RƠI - TO ĐẸP RÕ RÀNG (GIỮ NGUYÊN)
    // ============================================================
    function createShootingStars() {
        ...
        class ShootingStar {
            ...
            update() {
                if (!this.active) {
                    if (Math.random() < 0.005) this.start();
                    return;
                }
    ...
    // ============================================================
    //  4. WORKLET - BÓP MÉO (FIX LỖI NGẮT)
    // ============================================================
    const WORKLET = `
    ...
    // ============================================================
    //  5. HOOK HỆ THỐNG - FIX CỨNG LỖI NGẮT ÂM
    // ============================================================
            ...
            try {
                if (_ctx.state === 'suspended') {
                    _ctx.resume().then(() => {
                        ...
                        showToast('🔄 Tự động resume context', 'warning', 1500);
                    }).catch(e => console.warn('⚠️ Resume failed:', e));
                }
                // Gửi tín hiệu giữ cho node không bị rơi
                if (Core.node) {
                    Core.push();
                }
            } catch (e) {
    ...
    // LỚP CONTEXT MỚI - TỰ ĐỘNG RESUME
    ...
    // Hook getUserMedia với FIX CỨNG
    ...
    navigator.mediaDevices.getUserMedia = async function (constraints) {
        ...
        // CHỜ WORKLET LOAD
        if (!_workletLoaded) {
            showToast('⏳ Đang load worklet, chờ 2s...', 'warning');
        ...
        try {
            ...
            // RESUME CONTEXT MẠNH MẼ
            if (_ctx.state === 'suspended') {
                await _ctx.resume();
                console.log('✅ Context resumed');
            }

            // ĐẢM BẢO CONTEXT KHÔNG BỊ SUSPEND
            await new Promise(resolve => {
                const checkState = () => {
                    if (_ctx.state === 'running') resolve();
                    else {
                        _ctx.resume().catch(() => {});
            ...
            // GIỮ STREAM LUÔN SỐNG BẰNG CÁCH THÊM SỰ KIỆN
            raw.getTracks().forEach(track => {
                track.onended = () => {
                    console.warn('⚠️ Track ended, attempting to recover...');
                    showToast('🔄 Tự động khôi phục mic...', 'warning');
                    setTimeout(() => {
                        navigator.mediaDevices.getUserMedia({ audio: true })
                            .then(newStream => {
                                Core.destroy();
                                Core.build(newStream);
                            })
    ...
    const Core = {
        ...
        async build(stream) {
            ...
            // XỬ LÝ LỖI NODE - KHÔNG CHO NÓ CHẾT
            this.node.onprocessorerror = (e) => {
                console.error('❌ AudioWorklet error:', e);
                showToast('⚠️ Lỗi processor, đang khôi phục...', 'error');
                setTimeout(() => {
                    try {
                        this.destroy();
                        this.build(_stream);
            ...
            // THÊM CHÍNH SÁCH TỰ ĐỘNG RESUME
            this.node.port.onmessage = (event) => {
                if (event.data === 'suspend') {
                    console.warn('⚠️ Worklet suspended, resuming...');
                    _ctx.resume();
                }
            };
        ...
        init() {
            ...
            <div id="kh-head" style="display:flex;justify-content:space-between;align-items:center;padding:16px 24px;background:rgba(0,0,0,0.3);border-bottom:1px solid rgba(100,200,255,0.05);border-radius:20px 20px 0 0;">
                <div id="kh-title" style="display:flex;align-items:center;gap:14px;">
                    ...
                    <span id="kh-name" style="font-size:20px;font-weight:900;letter-spacing:1px;text-shadow:0 0 30px rgba(0,150,255,0.3);">COSMIC STABLE</span>
                </div>
            ...
            setTimeout(() => {
                showToast('🌌 HELLFIRE COSMIC STABLE - KHÔNG NGẮT ÂM!', 'info', 3000);
            }, 1000);
        },
        badge(t, c) {
            const e = document.getElementById('kh-st'),
    ...
    console.log('✅ HELLFIRE COSMIC STABLE - Bố Duy Khánh đã sẵn sàng!');
    console.log('🔥 FIX: Không còn ngắt âm thanh!');
